...
export function JobTab({ setGlobalJobDescription }: { setGlobalJobDescription: (desc: string) => void }) {
  ...
  const [activeLevel, setActiveLevel] = useState('Middle'); // <-- Стейт для открытой вакансии
  ...
  const handleLoadJob = () => {
    ...
    if (jobToLoad) {
      ...
      setActiveLevel(jobToLoad.level || 'Middle'); // Загружаем уровень
      setRegion(jobToLoad.region || 'Global'); // Загружаем регион
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1.5 uppercase tracking-wider">Level</label>
                  <select
                    value={activeLevel}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1.5 uppercase tracking-wider">Region</label>
                  <select
                    value={region}
